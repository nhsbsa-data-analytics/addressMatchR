  ...
  out = tryCatch(
    {
      ...
      non_exact_match_df <- primary_df %>%
        ...
        tidytext::unnest_tokens(., "TOKEN", primary_address_col, to_lower = FALSE, drop = FALSE) %>%
        dplyr::group_by(ID) %>%
        dplyr::mutate(
      ...
      lookup_tokens_df <- lookup_df %>%
        tidytext::unnest_tokens(., "TOKEN", lookup_address_col, to_lower = FALSE, drop = FALSE) %>%
        dplyr::distinct() %>%
        dplyr::mutate(TOKEN_WEIGHT = dplyr::if_else(grepl("[0-9]", TOKEN) == TRUE, 4, 1))
        ...
        dplyr::mutate(
          SCORE = dplyr::case_when(
            ...
            substr(TOKEN_LOOKUP, 1, 1) != substr(TOKEN, 1, 1) |
              ...
              stringr::str_detect(TOKEN_LOOKUP, TOKEN) != 1 ~ 0,
            T ~ stringdist::stringdist(TOKEN, TOKEN_LOOKUP, method = "jw", p = 0.1)
          ),
          # Remove scores of less than 0.8
          SCORE = dplyr::if_else(SCORE < 0.8, 0, SCORE),
          # Multiply the score by the token weight
          SCORE = SCORE * TOKEN_WEIGHT
        ) %>%
